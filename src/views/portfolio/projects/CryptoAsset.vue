<template>
  <div class="animated fadeIn" v-permission="PERMISSIONS.VIEW_PORTFOLIO">
    ...
    >
      <b-tab
        class="body-bg-color"
        style="border-left: 0; border-right: 0; border-top: 0; padding: 0"
        active
      >
...
export default {
    ...
    projectLabel() {
      if (this.cryptoAsset.hasOwnProperty('project')) {
        if (this.cryptoAsset.project.name && this.cryptoAsset.project.version) {
          return (
            this.cryptoAsset.project.name +
            ' ▸ ' +
            this.cryptoAsset.project.version
          );
        } else {
  ...
  mounted() {
    ...
    this.axios.get(cryptoAssetUrl).then((response) => {
      ...
      EventBus.$emit(
        'addCrumb',
        this.cryptoAssetLabel,
        'Project',
        this.cryptoAsset.project.uuid,
        this.projectLabel,
      );
